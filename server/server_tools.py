...
SPECS = [{
    ...
    "function": {
        ...
        "description": "横浜DeNAベイスターズの選手応援歌を、旋律つきで実際に"
                       ...
                       "「度会の歌歌って」「京田の応援歌」のように、人名らしき"
                       "言葉＋歌・応援歌の依頼が来たら（「歌って」と言われて"
                       "いなくても）、その名前に心当たりが無くても必ず"
                       "こちらを呼ぶ（歌える選手かはこの道具が判定して、"
                       "駄目なら error を返す。自分の知識だけで「歌えない」と"
                       "答えない。知らない名前は新加入の選手かもしれない）。"
    ...
    "function": {
        "name": "get_cheer_song",
        "description": "横浜DeNAベイスターズの選手応援歌の歌詞を文字で"
                       "調べて読み上げる。「歌詞を教えて」「どんな歌詞？」の"
                       "ように歌詞を求められた時だけ使う。「◯◯の応援歌」"
                       "「歌って」のように曲を聞きたい依頼は sing_cheer_song"
                       "（歌う方）。返ってきた歌詞は要約せず、そのまま読む。",
        "parameters": {
            "type": "object",
            "properties": {
                "player": {"type": "string",
