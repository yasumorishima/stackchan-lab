...
かたまりが長すぎる時の伸びは `split_long_runs` が別に見ている。**伸びるかどうかを決めるのは、句読点で切れた 1 かたまりのモーラ数**（実測: 33 モーラ以下は 0.12〜0.15 秒/モーラで健全、35 モーラ以上で 0.24〜0.48 に破綻。膝は 34 付近）。**字数では決まらない**（27 字で 0.351、26 字で 0.122 の実測がある）ので、`_mora_est` が読みのモーラ数を合成せずに見積り、`OJT_MAX_MORA`（既定 30）で切る。
