...
**旋律つきで歌う機能は入れたあと外した。** 実機で聴くと歌になっていなかった。全 25 曲を測ると **24 曲は起こした音符の 30〜57% が休み**で、短い音の連打にしかならない。基準を通った 1 曲も、聴けば駄目だった。**こちらの測り方（休みの割合・楽譜との平均ずれ）が耳と合っていなかった**ということなので、数値が通ったことを根拠に出さない。

**2026-08-05 に VOICEVOX の歌唱合成で作り直したが、リズムが元音源と合って
いないことが分かり、また外した。** 拍のずれは中央値 330〜1351ms（物差しは
較正済み: 同じ音源どうし 0ms・400ms 揺らした対照で 170ms）。原因は音符の
切れ目を「音の高さが変わった所」で決めていること＝歌のリズムは音節の始まりで
決まるので、同じ高さで続く歌詞が 1 音に潰れて以降がずれる。**歌詞の読み上げ
（`get_cheer_song`）は動く**。合成・採譜の部品と試験は残してある。以下は
その部品の記録。Sinsy 版の記録は `docs/progress.md`。
