...
| 読み上げ | Open JTalk（ローカル）。長い呼気段落は分けて合成し、読みが遅ければ実測して分け直します |
| 道具 | サーバー側 19 個（天気・週間天気・為替・株価指数・暗号資産・ニュース・地震・警報・台風・熱中症・電車・今日は何の日・月齢・燃油サーチャージ・渡航情報・プロ野球の速報と順位・出場選手の登録抹消・選手応援歌の歌詞・応援歌を歌う）＋ 本体側 11 個（画面・サーボ・カメラなど） |
| 歌 | 応援歌を旋律つきで歌います。メロディは公開されていないので、単旋律の音源から自分で採譜しました（[しくみ](server/README.md#歌う)） |
| 割り込み | 読み上げの途中で話しかけると、残りをやめて聞き直します。歌の途中でも同じです |
