...
| ページ | 内容 |
|---|---|
| [progress.md](progress.md) | セットアップと実装で踏んだ問題・その原因・測って決めたことの記録（日付順。もとは README にありました） |
| [server-history.md](server-history.md) | 自前サーバーを作った当時（2026-07-29〜30）の測定と判断。もとは `server/README.md` にありました |
...
ドキュメントではなくコードですが、音声まわりの現物はここにあります。

| 場所 | 内容 |
|---|---|
| [../server/](../server/) | OTA + WebSocket サーバー、ローカル音声認識（sherpa-onnx / Vosk / faster-whisper）、VOICEVOX での読み上げ、MCP クライアント、試験クライアントとモック |
